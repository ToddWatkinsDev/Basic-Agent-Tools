...
@tool
def draw_violin_plot(dataset_path, x_col, y_col, x_label, y_label):
    """Violin plot to see data density and distribution."""
    try:
        df = get_df(dataset_path)
        plt.figure(figsize=(10, 6))
        sns.violinplot(data=df, x=x_col, y=y_col)
        plt.title(f"{y_label} Distribution by {x_label}"); plt.xlabel(x_label); plt.ylabel(y_label)
        plt.savefig("violin_plot.png"); plt.close()
        return "Violin plot saved to violin_plot.png"
    except Exception as e: return str(e)

if __name__ == "__main__":
    if len(sys.argv) == 6:
        print(draw_violin_plot(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4], sys.argv[5]))
    else:
        print("Usage: python draw_violin_plot.py <dataset_path> <x_col> <y_col> <x_label> <y_label>")
